...
The title rule was chosen for one property: GitHub's auto-anchor for the new heading is **byte-identical to the old tag name**. GFM lowercases heading text and replaces spaces with hyphens, so `## Test driven development` → `#test-driven-development` — the old tag slug exactly. Verified mechanically over all **152** whole-line tag slugs in the corpus (brief, defs, routed docs, commands): 151 round-trip slug → title → anchor to the slug exactly, zero mismatches; the one documented exception is `quick_start` (noted above), whose anchor is `#quick-start` rather than byte-identical to its old tag — nothing cited it, so the exception cost nothing.
